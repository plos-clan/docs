...
这一章会把这三种场景分别练一遍。开始对比前记得先保存，因为 Diffview 主要读取磁盘上的工作树，没保存的内容可能根本不会显示。
...
上一章留下了一份半暂存状态。打开 `src/pocket_tasks/service.py`，保存以后按：

![半暂存的 service.py 同时显示 staged 与 unstaged 标记](screenshots/12-01-service-half-staged-before-diff.webp)

[[Space]] [[h]] [[d]]

屏幕上会并排出现 Git 快照和当前文件两个窗口。因为 `add_task` 已经进了 index，这次对照里主要应该看到还没暂存的 `completed_count`。
...
这套流程不依赖 Git。配置文件、生成结果、不同版本的文案，甚至接口响应，都可以直接摆在一起比较。
...
保存。现在两份文件都已经成了 Buffer，最后停在 `right.conf` 就行。
...
现在两个代码窗口分别显示 left 和 right。新分屏究竟出现在左边还是右边，会受当前布局影响，所以要看文件名确认内容，别只凭位置判断。
...
这两条命令都会修改目标 Buffer，但不会替你保存。真拿它们改文件时，先检查结果，再决定是保存还是撤销。

### 退出双文件 diff

在第一个窗口执行 [[F1]] → `diffoff` → [[Enter]] → [[Enter]]，移动到第二个窗口，再执行一次。随后可用 [[Ctrl]]+[[w]]、[[q]] 关闭其中一个分屏。

`compare` 目录只是这次练习的临时场地。想清理的话：
...
如果当前系统没有可用的回收站后端，Explorer 会永久递归删除整个目录。这里只删名字明确的练习目录；碰到真实目录时，一定要把路径和内容确认清楚以后再按 [[d]]。
...
同一条路径可能同时出现在 Staged 和 Unstaged 两个分组里。上一章留下的两个 `MM` 文件，正好能展示这种状态。
...
先反复按 [[Tab]] 把所有文件过一遍，再按 [[Space]] [[e]] 回到文件面板，用 [[j]]/[[k]] 和 [[Enter]] 精确选择。

![查看 test_service.py 的未暂存变化](screenshots/12-14-diffview-unstaged-test-service.webp)

在 Diffview 里，[[Space]] [[e]] 只在当前界面里生效：它负责把焦点切到文件面板，不会打开或关闭普通的 Explorer。

要修代码时，在对照窗口按 [[g]] [[f]]。Diffview 会继续留在自己的 Tab，真实文件则会在之前的工作 Tab 里打开。改完并保存以后，用 [[g]] [[t]] 或 [[g]] [[T]] 回到 Diffview，按 [[Space]] [[e]]，再按 [[R]] 刷新文件列表。
...
如果不小心在 Changes 视图的 index 一侧进入插入模式并保存，暂存区也会跟着变化。需要修改文件时，优先按 [[g]] [[f]] 回到普通工作 Tab。
...
这样会把上一章留下的 `completed_count` 及其测试也放进 index。如果暂存错了文件，把光标移到它的 Staged 条目，再按一次 [[s]]，就能取消整份文件的暂存。

### 一个需要了解但不必练习的高风险操作

> [!CAUTION] Diffview 文件面板的 X 键会直接恢复文件
> [[X]] 会把选中的文件恢复到 diff 左侧的状态，而且不会确认。插件虽然会短暂显示一条可能用于恢复的 Git 命令，但那条消息很容易错过。平时审查时不要随手按；确实要丢弃内容，优先回到普通文件预览 hunk，再逐块处理。

### 正确关闭 Diffview

普通的 [[q]] 只会影响当前窗口或面板，无法关闭整个 Diffview。
...
测试应该全部通过；`git diff --cached --check` 什么也没输出，就表示没发现空白错误。第三条命令会打开补丁，按 [[q]] 退出 pager。

![在 pager 中审查已暂存补丁](screenshots/12-20-cached-diff-reviewed.webp)

确认提交内容只有标题清理和完成数量以后，再运行：
...
注意最后的 [[D]] 是大写。这个映射实际比较的是“当前文件”和 `HEAD~`，也就是当前提交的父提交。它只看眼前这个文件，不会打开全项目 diff。

用 [[&#93;]] [[c]] 浏览刚提交的变化，之后关掉 Git 快照窗口。如果项目里只有根提交，`HEAD~` 还不存在，这个动作就会报找不到 revision。
...
不带参数的 `DiffviewFileHistory` 会查看整个项目的历史。那些需要精细 Git 参数的高级查询，等真正用到时再学也不迟，这份教程不要求提前掌握。
