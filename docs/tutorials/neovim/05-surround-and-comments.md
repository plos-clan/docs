...
所有练习都在 `model.py` 里完成。练完以后，文件还会恢复成上一章结尾的样子，不会留下任何临时修改。
...
如果文件和这里不一样，先照上一章结尾的内容改好，再用 [[F1]]、`write`、[[Enter]]、[[Enter]] 保存。
...
`mini.surround` 把 [[s]] 当成前缀，单独按 [[s]] 什么也不会做，这样就不容易误触。按下 [[s]] 以后，还得继续输入 [[a]]、[[d]]、[[r]]、[[h]]、[[f]] 等后续按键。

如果你想用 Vim 传统的“替换当前字符并进入插入模式”，按 [[c]] [[l]] 就行：[[c]] 负责修改，[[l]] 给出一个字符的范围。
...
现在再搜索一次 `/title`，确保光标落在引号里面，然后按 [[s]] [[d]] [["]]。

[[s]] [[d]] 表示准备删除包围，最后的 [["]] 指定双引号。完成后会恢复成：
...
包围操作同样支持点命令。以后要给好几个单词加引号，可以先做一次 [[s]] [[a]] [[i]] [[w]] [["]]，再移到下一处按 [[.]]，把刚才的操作重放一遍。
...
`mini.surround` 把开括号和闭括号分成了两种很实用的写法：
...
想要代码里常见的紧凑形式，最后就输入闭括号。

### 先看它认出了哪一对

把光标留在 `title` 上，按 [[s]] [[h]] [[)]]。

匹配到的左右圆括号会短暂亮一下。[[s]] [[h]] 只负责显示范围，不会改文件；遇到一层套一层的括号时尤其好用。
...
`mini.surround` 默认只处理光标所在的那对包围符。如果光标没在里面，它会提示找不到目标；把光标挪进去再试就行。
...
它会根据文件类型读取 `commentstring`。在 Python 文件里，它生成的是 `#` 注释；碰到 Treesitter 语言注入时，还能看光标落在哪儿，自动换成对应语言的注释格式。

切换多行注释时，规则是这样的：如果选区里的每个非空行都已经注释掉了，这次操作会统一取消注释；只要里面还混着普通代码，就会把整组选区一起注释掉。
...
次数要放在动作前面。[[3]] [[g]] [[c]] [[c]] 就是从当前行起处理三行，很适合临时关掉一小段连续代码。
...
如果注释范围没法用行数说清，可以先用 [[V]]、[[j]]、[[k]] 把选区框出来，再按 [[g]] [[c]]。

### 操作符版本

[[g]] [[c]] 也遵循上一章的“操作 + 范围”语法。按 [[g]] [[c]] [[j]]，意思就是切换当前行和下一行的注释。
...
范围一眼就能看清时，[[g]] [[c]] [[j]] 会比先进入可视模式更快。

## 8. 最终检查与保存

前面加过的包围和注释，现在都应该已经清理干净了。文件最后仍然是：
...
如果还有包围符没清掉，把光标移到里面，用 [[s]] [[d]] 加对应符号删除；如果还有注释，就把光标放到那一行，按 [[g]] [[c]] [[c]]。也可以顺着撤销记录多按几次 [[u]]，一步步退回去。
