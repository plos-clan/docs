...
这里汇总了整套课程的按键。平时按场景现查就好，不用把每一项都背下来。真正需要重点掌握的按键，会放在各节靠前的位置。

## 先认模式和记法

[[Space]] 是 Leader，[[,]] 是 Local leader。Leader 组合要依次输入，不用同时按住。按下 [[Space]] 后停大约半秒，Which-key 就会列出接下来能按哪些键。
...
碰到“按键毫无反应”，先按一次 [[Esc]] 回到普通模式，再重新试。Leader 组合里的每个键要在大约 500 毫秒内接着输入，停得太久，Neovim 就会当成这组按键已经结束。

## 日常开发主流程

这张表覆盖了一轮常见的开发流程。先熟悉这里，后面的表按需翻就行。
...
[[F1]] 只在普通模式下有映射。输入命令名以后，第一次按 [[Enter]] 会选中命令，把它填进底部命令行；第二次按 [[Enter]] 才会真正运行。命令前面的冒号由 Picker 自动补上。
...
文件不会自动保存。由于 swap、backup、writebackup 和持久化 undo 文件默认也是关闭的，因此突然断电或进程异常退出时，没保存的内容可能就找不回来了，所以记得经常保存。
...
mini.ai 使用 `cover_or_next` 搜索策略。光标不在对象内部时，它可能会继续寻找后面大约 50 行内的下一个对象。真要删除或修改之前，最好先用可视模式看清选区，免得改到远处的参数。
...
mini.pairs 会在插入模式下自动补上常见的右括号和右引号。碰到已经存在的右括号时，留意光标是直接越过去了，还是又插入了一个相同字符。

mini.surround 默认使用 `cover` 搜索策略，也就是说，目标包围符必须盖住光标位置。用 [[s]] [[d]]、[[s]] [[r]] 之前，可以先按 [[s]] [[h]] 看看目标；如果找不到，就把光标挪进那对包围符里再试。
...
Buffer 里有没保存的修改时，会弹出确认框。选择 `No` 会丢掉修改并继续删除；选择 `Cancel` 则什么也不做。
...
在 Explorer 里按 [[/]]，会进入递归文件名筛选。选中结果后，第一次 [[Enter]] 只是回到文件树并定位，第二次 [[Enter]] 才会打开文件。
...
标准的多选移动流程是：在每个来源项上按 [[Tab]]，再把光标移到目标目录，按 [[m]]，核对确认框后确认。光标停在文件上时，“当前目录”指的是文件的父目录；停在目录上时，指的就是目录本身。如果一个 [[Tab]] 标记都没有，[[m]] 会打开重命名框，所以它不能单独充当“移动当前项”按钮。

当前系统的 PATH 里没有 Snacks 能用的 `trash`、[[g]] [[i]] [[o]] 或 `kioclient`，所以 Explorer 会退回到递归永久删除。按 [[d]] 之前先核对名称；用 [[Tab]] 多选以后，还要再看一遍整组名单。
...
[[Ctrl]]+[[c]] 会改变后续 grep、终端和会话使用的目录。如果搜索范围突然缩小，先想想自己是不是在 Explorer 里按过这组键。

Diffview 里的 [[Space]] [[e]] 只会把焦点切到 Diffview 文件面板，不能用来开关全局 Explorer。关掉 Diffview 以后，它才会恢复原来的含义。
...
关闭 Window 只是关掉当前显示区域，Buffer 可能还留在内存里。需要管理 Buffer 时，用 [[Space]] [[,]]。顶部页签显示的是 Tab 布局，不会把所有 Buffer 一一列出来。

## Blink 补全、签名和 Copilot

### Blink 补全

补全来源包括 LSP、路径、friendly-snippets 和当前 Buffer 里的文字。就算某种文件没有语言 LSP，也仍然可能看到路径、片段和文字候选。
...
[[Tab]] 被设成了直接接受当前项。按下去之前先看一眼候选内容，免得插入的东西和预想不一样。

### Copilot 灰字建议

要使用这些按键，Copilot 必须已经认证，网络也得可用，而且当前 Buffer 已经加载了 Copilot。
...
有些终端或桌面环境会截走 Alt 组合。如果按键根本没送到 Neovim，先检查终端的快捷键设置。

## LSP：定义、引用、诊断和重构

下面这些映射，只有 LSP 成功附着到当前 Buffer 后才会出现，而且都在普通模式下使用。按 [[Space]] [[l]] 后停一下，可以从 Which-key 里查看当前有哪些分支。
...
Neovim 0.12 还自带一组 LSP 映射。它们同样只会在相应能力可用时出现：
...
平时优先用 [[Space]] [[c]] [[f]]。它会异步修改当前 Buffer，但不会保存文件。等格式变化完成以后，再用 [[F1]] → `write` →两次 [[Enter]]。
...
测试、构建、Git 提交和 `git rebase --continue`，都可以直接在右侧终端里运行。

## Gitsigns：逐块处理改动

新文件第一次 `git add` 之前，不会有正常的 Gitsigns gutter hunk 效果，因为 `attach_to_untracked` 处于关闭状态。
...
在 diff Window 里，[[&#93;]] [[c]] 和 [[&#91;]] [[c]] 会自动改用 Neovim 原生的差异块跳转。
...
[[Space]] [[h]] [[r]] 和 [[Space]] [[h]] [[R]] 都不会弹确认框，而是立刻改写 Buffer。大写 [[R]] 会影响整个文件，动手前一定要确认目标和修改范围。
...
Which-key 把 [[Space]] [[h]] [[D]] 写成了 `Diff project`，不过它实际只会比较当前文件和父提交。要审查整个项目，请用 [[Space]] [[g]] [[s]]。
...
[[d]] [[o]] 和 [[d]] [[p]] 都会改动文本。拿不准方向时，先保存，或者留一份提交作为基线。

## Diffview：全项目审查

进入之前先把相关文件全部保存。Diffview 读取的是工作树状态，没保存的内容可能不会出现在对照结果里。
...
[[q]] 无法关闭整个 Diffview；它在不同子窗口中具有不同作用。要完整退出，请使用 `DiffviewClose`。
...
[[X]] 不会弹确认框，而是直接还原文件。插件会把旧内容写入 Git object database，也会显示一条可能用于恢复的命令，不过通知很容易错过，所以使用前必须确认文件和版本。

“Changes”组左侧是 Git index，右侧是 working tree；“Staged changes”组左侧是 HEAD，右侧是 Git index。index 所在的一侧可以编辑，保存后会直接改变暂存区。如果只是看差异，就别在 index 那边修改内容。

## Merge 与 Rebase 冲突

仓库正处在 merge 或 rebase 冲突状态时，打开 Diffview 就会进入三方布局。
...
大写命令和 [[d]] [[X]] 都会作用于整份文件。rebase 期间，Git 的 ours 通常指目标分支状态，theirs 通常指眼前正在应用的提交，和普通 merge 时的直觉可能相反。

这些选择动作会读取结果文件里的冲突标记。如果没有 `|||||||` BASE 标记，[[b]]/[[B]] 会把 BASE 当成空内容，可能清空当前冲突，甚至当前文件里的全部冲突；[[a]]/[[A]] 则会拼接现有的 ours 和 theirs。即便 BASE 标记存在，只要内容为空，[[b]]/[[B]] 留下的结果也会是空的。按这四个键之前，先确认 BASE 段是否存在，里面又有什么内容。
...
在项目 grep Picker 里按 [[Ctrl]]+[[q]]，可以把结果送进 Quickfix。
...
Quicker 默认允许编辑。写入 Quickfix 会跨文件修改项目，还会自动保存那些原本处于未修改状态的目标 Buffer。`wqall` 也可能触发这次写回。把 Quickfix 编辑当成一次批量重构：先提交基线，再动手。
...
[[Space]] [[s]] [[l]] 同时也是 [[Space]] [[s]] [[l]] [[t]] 的前缀，所以只按前者时，可能要等大约 500 毫秒。

会话会保存 cwd、文件 Buffer、Window、Tab 和折叠布局。没写进磁盘的文本不会进入会话文件。[[Space]] [[s]] [[c]] 只是一份布局快照，代替不了保存文件。

退出时会尝试保存当前 cwd 的会话；不带参数启动时，则会自动载入全局最近会话。要明确进入某个项目，可以这样运行：
...
带上目录参数以后，它不会自动载入最近会话，而是直接打开当前项目的 Explorer。

载入另一份会话时，会删除当前这组 Buffer。确认框里的选项含义如下：
...
保存会话之前，配置会先关闭全部 Diffview。Session Manager 还会清理那些无法恢复的工具 Buffer，所以手动保存会话时，Explorer、Quickfix 或其他插件面板可能会消失。

## 语言能力矩阵

“语法”指 Treesitter 高亮和缩进解析；“LSP”指语言级补全、诊断、导航和重构入口。具体能做什么，仍然取决于服务器对当前语言和项目的支持。
...
全局默认的 `shiftwidth` 和 `tabstop` 都是 8，而且使用空格缩进。Python ftplugin 通常会设成 4，JSON 和 YAML 通常会设成 2。Sleuth 会根据现有文件判断缩进，`.editorconfig` 则能提供更可靠的项目规则。如果新建空文件时出现了 8 个空格的缩进，说明当前用的还是全局默认值。

文件大于约 1.5 MB，或者平均每行特别长时，会进入 Snacks bigfile 模式。LSP、Treesitter 这类开销较大的功能可能会被关闭，这是为了保护性能。

## 高风险操作总表

> [!CAUTION] 执行前先保存并确认 Git 基线
> 表里的操作可能删除文件、丢掉没保存的修改，或是批量改写内容。动手前先保存、预览，并确认手头有可恢复的 Git 基线。
...
Python 的 BasedPyright 只分析已经打开的文件。某个没打开的模块没有显示诊断，是当前 `openFilesOnly` 设置下的正常现象。
...
未跟踪文件不会附着完整的 Gitsigns。先在终端运行 `git add 文件名 `，再回到 Neovim；必要时重新打开 Buffer。

:::

::: details Diffview 内容落后

先保存工作文件，再到 Diffview 文件面板里按 [[R]]。进入 Diffview 前跑一次 [[F1]] → `wall` → 两次 [[Enter]]，可以避免没保存的修改漏出对照结果。

:::

::: details 按 q 无法关闭 Diffview
...
这是 Snacks bigfile 在保护交互速度。文件超过约 1.5 MB，或者平均每行特别长时，这些能力会被主动关掉。
...
到这里，课程主线就全部结束了。你可以回到 [课程目录](./) 挑几章再练，也可以直接打开真实项目，跟着实际需求继续用。接下来最重要的，就是反复使用这些操作，把它们慢慢变成稳定习惯。
