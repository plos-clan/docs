...
按 `gt` 或 `gT` 前往 `cli.py` 所在 Tab。若第 07 章的 Tab 已经关闭，就按 `Space ,`，输入 `cli`，选中 `src/pocket_tasks/cli.py` 后按 `Enter`。
...
若内容有少量差异也没关系，先把本章练习涉及的几行调整到示例状态。保存时使用：

```text
Esc → F1 → 输入 write → Enter → Enter
```
...
2. 按 `0` 到行首，再按 `cc`。当前行内容被清空，Neovim 进入插入模式，并保留 Python 缩进。
3. 输入 `tasks: li`，先停一下。
...
若担心其他 Window 里还有修改，执行：

```text
F1 → 输入 wall → Enter → Enter
```
