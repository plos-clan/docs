...
3. 光标落到 `created_at: datetime` 后按 `dd`。
4. 输入 `/from datetime`，按 `Enter`，再按 `dd` 删除已经无用的导入。
...
8. 按 `Enter` 打开它，光标落在 `- \`created_at\`` 上后按 `dd`。
