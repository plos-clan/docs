...
例如，`d` 表示删除，`w` 表示移动到下一个词；二者组合成 `dw`，意思就是“删除到下一个词”。掌握这条规则后，许多快捷键都可以按含义组合，不必逐个死记。
...
| 组合 | 逐键解释 | 效果 |
| --- | --- | --- |
| `dw` | `d` 删除，`w` 到下一词 | 删除到下一个词 |
| `d$` | `d` 删除，`$` 到行尾 | 删除到行尾 |
| `cw` | `c` 修改，`w` 到下一词 | 删除词尾范围并进入插入模式 |
| `yw` | `y` 复制，`w` 到下一词 | 复制到下一个词 |
...
| --- | --- |
| `dd` | 删除当前整行 |
| `cc` | 清掉当前行内容，保留合适的缩进并进入插入模式 |
| `yy` | 复制当前整行 |

前面可加次数：`2dd` 删除两行，`3yy` 复制三行。
...
删除和修改的内容也会进入寄存器，所以 `dd` 后按 `p`，可以把那一行挪回来。

### 删除一行再恢复

1. 保持光标在 `completed` 行，按 `dd`。
2. 这一行消失。
...
| 对象 | `i` 版本 | `a` 版本 |
| --- | --- | --- |
| 单词 word | `iw` 取词本身 | `aw` 通常连相邻空白一起取 |
| 双引号 | `i"` 取引号里的内容 | `a"` 连双引号一起取 |
| 圆括号 | `i(` 或 `i)` 取括号内部 | `a(` 或 `a)` 连括号一起取 |
| 任意常见括号 | `ib` | `ab`，`mini.ai` 会在 `()`、`[]`、`{}` 中找平衡的一对 |
...
`ciw` 是日常编辑中最常用的组合之一。只要光标位于单词内部，具体停在哪个字母上都没有关系。
...
| 对象字母 | 代表什么 | 示例 |
| --- | --- | --- |
| `a` | 函数调用中的一个 argument | `cia` 修改当前参数内部 |
| `f` | 整个 function call | `vaf` 选中函数名与括号 |
| `q` | 最近的一对单引号、双引号或反引号 | `ciq` 修改任一种引号内部 |
| `t` | HTML/XML 标签 | `vit` 选中标签内容 |

这里的字母会根据语法位置解释。`cia` 可以拆成：
...
`mini.ai` 识别了 `dataclass(...)` 中的参数 `slots=False`，`cia` 会选择整个参数内容供你修改。若要同时包含参数旁边的分隔空白或逗号，可以使用 around 版本 `caa`。
...
- `.` 在新位置重放 `ciw` 加输入 `created`，把 `completed` 也变成 `created`。
